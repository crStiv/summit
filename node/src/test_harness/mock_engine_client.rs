use alloy_rpc_types_engine::{
    ...
    ExecutionPayloadV3, ForkchoiceState, PayloadId, PayloadStatus, PayloadStatusEnum, BlobsBundleV1
};
use alloy_eips::eip7685::Requests;
use alloy_primitives::{Address, Bloom, Bytes, FixedBytes, U256, B256};
...
impl MockEngineClient {
    ...
    pub fn get_canonical_chain(&self) -> Vec<(u64, FixedBytes<32>)> {
        let state = self.state.lock().unwrap();
        let mut chain: Vec<_> = state.canonical_by_number.iter()
...
impl MockEngineState {
    ...
    fn create_block_payload(&self, parent_hash: FixedBytes<32>, timestamp: u64, client_id: &str) -> ExecutionPayloadV3 {
        ...
        let payload_v1 = ExecutionPayloadV1 {
            ...
            gas_used: 21_000,
            timestamp,
            extra_data: Bytes::from(format!("block-{}-{}", self.next_block_number, client_id).into_bytes()),
...
impl EngineClient for MockEngineClient {
    ...
    async fn check_payload(&self, block: &Block) -> PayloadStatus {
        ...
        state.known_blocks.insert(block_hash, status.clone());
        state.validated_blocks.insert(block_hash, block.payload.clone());
        status
    }
    ...
    async fn commit_hash(&self, fork_choice_state: ForkchoiceState) {
        ...
        let matching_block = state.building_payloads
            .values()
            .find(|envelope| {
        ...
        if let Some(block) = matching_block {
            ...
            let validated_block = state.validated_blocks.get(&fork_choice_state.head_block_hash).cloned();
        ...
        for client in clients.iter().skip(1) {
            ...
            if client_height != reference_height {
                return Err(format!(
                    ...
                    clients[0].client_id(), reference_height,
                    client.client_id(), client_height
                ));
            }
            ...
            if client_chain != reference_chain {
                return Err(format!(
                    ...
                    client.client_id(), clients[0].client_id()
                ));
            }
        }
...
mod tests {
    ...
    async fn test_multiple_clients_consensus() {
        ...
        assert!(network.verify_consensus().is_ok());
    ...
    async fn test_client_divergence_and_convergence() {
        ...
        let payload_id = client1.start_building_block(genesis_state, 1000).await.unwrap();
        ...
        assert!(network.verify_consensus().is_err());
    ...
    async fn test_multiple_block_production() {
        ...
        for round in 1..=3 {
            ...
            let payload_id = producer.start_building_block(fork_choice, (round * 1000) as u64).await.unwrap();
    ...
    async fn test_consensus_failure_scenarios() {
        ...
        let payload_id_a = client1.start_building_block(genesis_state, 1000).await.unwrap();
        ...
        let payload_id_b = client2.start_building_block(genesis_state, 1000).await.unwrap();
